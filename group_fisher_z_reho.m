...
% spearman's corr. coeff. btw. ReHo and number of spikes,
% and assemble an array with format given below for every subject,
% col. 1 = event type
% col. 2 = channel name
% col. 3 = rho_all runs
% col. 4 = p-value
% col. 5 = Fisher's z all runs (computed in this script)
...
op_filename = 'fisher_z_reho_all_subs.mat';
...
fname_reho_struct = [directname, filesep, 'matrices' filesep 'reho_spikes_fixed_winsize'];   % direct. of reho struct is located
filename_reho_struct = 'reho_spikes_fixed_windsize.mat';   % filename of reho struct. file
...
[~, reho_struct_path, ~] = get_path(fname_reho_struct, filename_reho_struct);   % reho
...
if isempty(reho_struct_path) || isempty(spikes_stats_struct_path)
    return
end
...
st_reho = load(reho_struct_path{:});   % reho struct.
st_spikes_stats = load(spikes_stats_struct_path{:});   % spikes stats struct.

% access fieldnames of struct. terms
fds_reho = fieldnames(st_reho.terms);   % reho struct.
fds_spikes_stats = fieldnames(st_spikes_stats.terms);   % spikes stats struct.

% get non-empty fieldnames for reho and spikes_stats struct.
fd_ind_reho = [];   % initialize array for storing indices of non-empty fields
for sess_ind = 1:length(fds_reho)   % for each field
    if ~isempty(st_reho.terms.(fds_reho{sess_ind}))   % check if field is empty
        fd_ind_reho = [fd_ind_reho sess_ind];   % if not, append index to array
    end
end
...
fdnames_reho = fds_reho(fd_ind_reho);   % reho
fdnames_spikes_stats = fds_spikes_stats(fd_ind_spikes_stats);   % spikes stats

% variable interested: 
% spearman's rho and p-value btw. ReHo and number of spikes (A),
...
rho_pval_comb_all = st_reho.terms.(fdnames_reho{1}).rho_pval_comb_all;   % spearman rho and p-value
