...
% format path where input struct. is stored at
...
fname_input_falff = [subnum_dir, filesep, 'matrices' filesep 'dynamic_analysis_falff']; 
filename_input_falff = 'dynamic_analysis_falff_fisher_z.mat';   % filename of falff file

% enter path where plots are saved at
% directname_op = '/work/levan_lab/mtang/fmri_project/plots/dynamic_analysis_alff_falff_plot';
directname_op = 'C:\Users\siumichael.tang\Downloads\fmri_project\plots\dynamic_analysis_reho_alff_falff_plot';

% format output filename
filenameg_op = ['dynamic_analysis_reho_alff_falff_plot'];
...
file_path_falff = fullfile(fname_input_falff, filename_input_falff);

% load .mat file
reho_st = load(file_path_reho);
alff_st = load(file_path_alff);
falff_st = load(file_path_falff);
...
ch_onset_ds_ied_falff_all = collect_ar(falff_st.main, 'ch_onset_ds_ied_falff_comb');
ch_propa_ds_ied_falff_all = collect_ar(falff_st.main, 'ch_propa_ds_ied_falff_comb');
ch_ext_ds_ied_falff_all = collect_ar(falff_st.main, 'ch_ext_ds_ied_falff_comb');
...
% for falff, 
[ch_onset_coef_var_falff, ch_onset_fisher_z_falff] = ...
    remove_nan_inf(ch_onset_ds_ied_falff_all(:, end-1), ch_onset_ds_ied_falff_all(:, end));
[ch_propa_coef_var_falff, ch_propa_fisher_z_falff] = ...
    remove_nan_inf(ch_propa_ds_ied_falff_all(:, end-1), ch_propa_ds_ied_falff_all(:, end));
[ch_ext_coef_var_falff, ch_ext_fisher_z_falff] = ...
    remove_nan_inf(ch_ext_ds_ied_falff_all(:, end-1), ch_ext_ds_ied_falff_all(:, end));
...
% [f1, ax11, ax12] = make_plot(ch_onset_coef_var_alff, ch_onset_fisher_z_alff, ...
%     ch_onset_coef_var_falff, ch_onset_fisher_z_falff, 'onset', ...
%     op_results, directname_op, filenameg_op);   % onset channels
% [f2, ax21, ax22] = make_plot(ch_propa_coef_var_alff, ch_propa_fisher_z_alff, ...
%     ch_propa_coef_var_falff, ch_propa_fisher_z_falff, 'propa', ...
%     op_results, directname_op, filenameg_op);   % propa channels
[f3, ax31, ax32, ax33] = make_plot(ch_ext_coef_var_reho, ch_ext_fisher_z_reho, ...
    ch_ext_coef_var_alff, ch_ext_fisher_z_alff, ...
    ch_ext_coef_var_falff, ch_ext_fisher_z_falff, 'ext', ...
...
% dynamic correlation vs (X) coefficient of variation of falff (left) and 
% alff (right),
...
% coef_var_alff = coefficient of variation (cov) of ied rates from alff analysis
% fisher_z_alff = fisher-transformed dynamic corr. btw. alff and cov
% coef_var_falff = coefficient of variation (cov) of ied rates from alff analysis
% fisher_z_falff = fisher-transformed dynamic corr. btw. alff and cov
...
% ax2 = gca for alff plot (left)
% ax3 = gca for falff plot (right)
function [f1, ax1, ax2, ax3] = make_plot(coef_var_reho, fisher_z_reho, ...
    coef_var_alff, fisher_z_alff, coef_var_falff, ...
    fisher_z_falff, ch_cat_str, op_results, directname_op, filenameg_op)
...
% ax1.FontSize = 12;
ax1.FontSize = 9;   % axes font size
ax1.YLabel.FontSize = 15;   % y-label font size
ax1.XLabel.FontSize = 12;   % x-label font size
t1.FontSize = 13;   % title font size
...
if xlabel_p_val_2 >= 0.001
    xlabel(sprintf('\\((\\sigma/\\mu)_{\\mathrm{ALFF}}\\), \\(r = %.3f, p = %.3f\\)', ...
...
% set axis fontsize (default = 10)
% ax2.FontSize = 12;
ax2.FontSize = 9;   % axes font size
ax2.YLabel.FontSize = 15;   % y-label font size
ax2.XLabel.FontSize = 12;   % x-label font size
t2.FontSize = 13;   % title font size
...
% add scatterplot (X): coeff. of var., (Y): fisher z-score of falff vs ied
% rate
scatter(coef_var_falff, fisher_z_falff, sz, 'filled', ...
    'MarkerFaceColor', [0 0 0.7]); hold on

% call get_regression_line to obtain predicted y data, pearson's corr.
% coeff., and its p-value based on (X) and (Y)
[y_pred_falff, pear_rho_falff, p_val_falff] = get_regression_line(coef_var_falff, fisher_z_falff);

% plot regression line
plot(coef_var_falff, y_pred_falff, '-k', 'LineWidth', 1.15); hold off
...
xlabel_rho_3 = round(pear_rho_falff, 3);   % corr. to 3 d. p.
xlabel_p_val_3 = round(p_val_falff, 1, 'significant');   % corr. to 1 sig. fig.

if xlabel_p_val_3 >= 0.001
    xlabel(sprintf('\\((\\sigma/\\mu)_{\\mathrm{fALFF}}\\), \\(r = %.3f, p = %.2f\\)', ...
        xlabel_rho_3, xlabel_p_val_3), 'Interpreter','latex');
else
    xlabel(sprintf('\\((\\sigma/\\mu)_{\\mathrm{fALFF}}\\), \\(r = %.3f, p < 0.001\\)', ...
        xlabel_rho_3), 'Interpreter','latex');
end

% ylabel('Dynamic correlation','Interpreter','latex');

% format title:
falff_mean_fisher_z = mean(fisher_z_falff);   % get mean Fisher'z
[~, falff_mean_fisher_z_pval] = ttest(fisher_z_falff);   % get p-value of fisher z
falff_mean_fisher_pval_disp = round(falff_mean_fisher_z_pval, 1, 'significant');   % round numbers
t3 = title(sprintf('fALFF (\\(\\bar{z} = %.3f, p = %.2f\\))', falff_mean_fisher_z, ...
    falff_mean_fisher_pval_disp),'Interpreter','latex');
...
% format axes
ax3.XLim = [xmin,xmax];   % x-limits
ax3.XTick = [xmin:xtick:xmax];
    
ax3.YLim = [ymin,ymax];   % y-limits
ax3.YTick = [ymin:ytick:ymax];
  
% set axis fontsize (default = 10)
ax3.FontSize = 9;
ax3.YLabel.FontSize = 15;
ax3.XLabel.FontSize = 12;
t3.FontSize = 13;   % title font size
...
% adjust title position after setting aspect ratio of plot
t3.Position(2) = t3.Position(2) + 0.04;

% end subplot for falff
%----------------------------------------------
...
ax3.Position(1) = ax2.Position(1) + ax2.Position(3) + hdiff;
ax3.Position(2) = ax2.Position(2);
...
end   % end function [f1, ax1] = make_plot(coef_var_falff, fisher_z_falff, coef_var_alff, fisher_z_alff)
