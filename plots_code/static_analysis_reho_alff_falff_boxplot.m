...
%---------------------------------------------------------------------------
% Drawn on 'static_analysis_reho.mat', 'static_analysis_alff.mat' and 
% 'static_analysis_falff.mat', which
% have
% (i) ReHo, ALFF or fALFF, depending on filename 
...
% where var denotes the fmri feature interested and was calculated for each session.
...
filename_input_falff = 'static_analysis_falff_1.mat';   % filename of input file of falff
...
filenameg_op = ['static_analysis_reho_alff_falff_boxplot'];
...
% load structure of reho, alff and falff
...
st_falff = load(fullfile([directname, filesep, 'static_analysis_falff'], filename_input_falff));
...
fd_name_sub_falff = fieldnames(st_falff.grand);

% (i) ReHo, (ii) ALFF, (iii) fALFF, (iv), IED_{long term}, (v) IED_{during scan}, 
% were computed in each run for every subject, and their spearman's corr.
% coeff. was computed and classified into the following categories based on
% channel type. 
% For ReHo/ALFF/fALFF - IED_{long term}, channel types are onset,
% propagation, no ied, and all = onset + propa + no ied
% For ReHo/ALFF/fALFF - IED_{during scan}, channel types are onset,
...
% For instance,
% rho_alff_ied_lt_onset_st = strcut of rho btw. ALFF and IED_{long term} for ied onset
% channels

% for ReHo/ALFF/fALFF - IED_{long term}, we have, 
...
rho_falff_ied_lt_onset_st = process_var(st_falff.grand, 'rho_falff_ied_lt_onset');
rho_falff_ied_lt_propa_st = process_var(st_falff.grand, 'rho_falff_ied_lt_propa');
rho_falff_ied_lt_noied_st = process_var(st_falff.grand, 'rho_falff_ied_lt_noied');
rho_falff_ied_lt_all_st = process_var(st_falff.grand, 'rho_falff_ied_lt_all');

% for ReHo/ALFF/fALFF - IED_{during scan}, we have, 
...
rho_falff_ied_ds_onset_st = process_var(st_falff.grand, 'rho_falff_ied_ds_onset');
rho_falff_ied_ds_propa_st = process_var(st_falff.grand, 'rho_falff_ied_ds_propa');
rho_falff_ied_ds_ext_st = process_var(st_falff.grand, 'rho_falff_ied_ds_ext');

% format channel category str required for make_plot function defined below
ch_cat_str_all = {'rho_reho_ied_lt_all', 'rho_alff_ied_lt_all', 'rho_falff_ied_lt_all', ...
    'rho_reho_ied_ds_ext', 'rho_alff_ied_ds_ext', 'rho_falff_ied_ds_ext'};   % all channels

ch_cat_str_onset = {'rho_reho_ied_lt_onset', 'rho_alff_ied_lt_onset', 'rho_falff_ied_lt_onset', ...
    'rho_reho_ied_ds_onset', 'rho_alff_ied_ds_onset', 'rho_falff_ied_ds_onset'};   % ied onset channels

ch_cat_str_propa = {'rho_reho_ied_lt_propa', 'rho_alff_ied_lt_propa', 'rho_falff_ied_lt_propa', ...
    'rho_reho_ied_ds_propa', 'rho_alff_ied_ds_propa', 'rho_falff_ied_ds_propa'};   % ied propa channels

ch_cat_str_noied = {'rho_reho_ied_lt_noied', 'rho_alff_ied_lt_noied', 'rho_falff_ied_lt_noied'};   % no ied channels
...
[f1, ax1, rho_reho_ied_lt_all_st, rho_reho_ied_ds_all_st, ...
    rho_alff_ied_lt_all_st, rho_alff_ied_ds_all_st, rho_falff_ied_lt_all_st, ...
    rho_falff_ied_ds_all_st] = make_plot(st_reho, st_alff, st_falff, ch_cat_str_all, ...
...
%---------------------------------------------------------------
%---------------------------------------------------------------
% FUNCTION [F1, AX1, RHO_REHO_IED_LT_CAT_ST, ...] = MAKE_PLOT(ST_REHO, ST_ALFF,
% ST_fALFF, ...)
% this function makes boxplot of static corr. btw. (1) reho - ied_{long term},
% (2) reho - ied_{during scan}, (3) ALFF - ied_{long term},
% (4) ALFF - ied_{during scan}, (5) fALFF - ied_{long term}, (6) fALFF - ied_{during scan},
...
% ST_fALFF = struct obtained from static analysis of falff
...
% RHO_fALFF_IED_LT_CAT_ST = struct listing variables required for boxplot,
% for static corr. btw. falff and ied_{long term}, see process_var for details, 
% RHO_fALFF_IED_DS_CAT_ST = struct listing variables required for boxplot,
% for static corr. btw. falff and ied_{during scan}, see process_var for details, 

function [f1, ax1, rho_reho_ied_lt_cat_st, rho_reho_ied_ds_cat_st, ...
    rho_alff_ied_lt_cat_st, rho_alff_ied_ds_cat_st, rho_falff_ied_lt_cat_st, ...
    rho_falff_ied_ds_cat_st] = make_plot(st_reho, st_alff, st_falff, ch_cat_str, op_results, ...
...
rho_falff_ied_lt_cat_st = [];   % falff - ied_{long term}
rho_falff_ied_ds_cat_st = [];   % falff - ied_{during scan}

% get struct containing all variables required for boxplot using
% process_var, see end of script for details
if numel(ch_cat_str) == 6
rho_reho_ied_lt_cat_st = process_var(st_reho.grand, ch_cat_str{1});   % reho - ied_{long term}
rho_alff_ied_lt_cat_st = process_var(st_alff.grand, ch_cat_str{2});   % alff - ied_{long term}
rho_falff_ied_lt_cat_st = process_var(st_falff.grand, ch_cat_str{3});   % falff - ied_{long term}

rho_reho_ied_ds_cat_st = process_var(st_reho.grand, ch_cat_str{4});   % reho - ied_{during scan}
rho_alff_ied_ds_cat_st = process_var(st_alff.grand, ch_cat_str{5});   % alff - ied_{during scan}
rho_falff_ied_ds_cat_st = process_var(st_falff.grand, ch_cat_str{6});   % falff - ied_{during scan}

% from each struct, access variables required, fisher z-scores, mean fisher
% z score, and the corresponding p-value, for each column in boxplot (alff
% - ied_{long term}, alff - ied_{during scan}, etc.
...
rho_falff_ied_lt_cat_fisher_z = rho_falff_ied_lt_cat_st.fisher_z;
rho_falff_ied_ds_cat_fisher_z = rho_falff_ied_ds_cat_st.fisher_z;
...
rho_falff_ied_lt_cat_mean_fisher_z = rho_falff_ied_lt_cat_st.mean_fisher_z;
rho_falff_ied_ds_cat_mean_fisher_z = rho_falff_ied_ds_cat_st.mean_fisher_z;
...
rho_falff_ied_lt_cat_pval_fisher_z = rho_falff_ied_lt_cat_st.pval_fisher_z;
rho_falff_ied_ds_cat_pval_fisher_z = rho_falff_ied_ds_cat_st.pval_fisher_z;
end
...
rho_falff_ied_lt_cat_st = process_var(st_falff.grand, ch_cat_str{3});
...
rho_falff_ied_lt_cat_mean_fisher_z = rho_falff_ied_lt_cat_st.mean_fisher_z;
rho_falff_ied_ds_cat_mean_fisher_z = [];
...
rho_falff_ied_lt_cat_pval_fisher_z = rho_falff_ied_lt_cat_st.pval_fisher_z;
rho_falff_ied_ds_cat_pval_fisher_z = [];
end
...
g5 = repmat({'fALFF-IED_lt'}, length(rho_falff_ied_lt_cat_fisher_z), 1);   % falff - ied_{long term}
g6 = repmat({'fALFF-IED_ds'}, length(rho_falff_ied_ds_cat_fisher_z), 1);   % falff - ied_{during scan}
else   % when there are not 4 entries (during scan ied rate is missing for the channel category)
g1 = repmat({'ReHo-IED_lt'}, length(rho_reho_ied_lt_cat_fisher_z), 1);   % reho - ied_{long term}
g2 = [];
g1 = repmat({'ALFF-IED_lt'}, length(rho_alff_ied_lt_cat_fisher_z), 1);   % alff - ied_{long term}
g2 = [];
g3 = repmat({'fALFF-IED_lt'}, length(rho_falff_ied_lt_cat_fisher_z), 1);   % falff - ied_{long term}
g4 = [];
end

g = [g1; g2; g3; g4; g5; g6];   % concatenate them together
...
xlabel_str = format_xlabel_str(rho_reho_ied_lt_cat_pval_fisher_z, ...
    ...
    rho_alff_ied_ds_cat_pval_fisher_z, rho_falff_ied_lt_cat_pval_fisher_z, ...
    rho_falff_ied_ds_cat_pval_fisher_z);
...
display([rho_reho_ied_lt_cat_mean_fisher_z rho_reho_ied_ds_cat_mean_fisher_z, ...
    ...
    rho_falff_ied_lt_cat_mean_fisher_z, rho_falff_ied_ds_cat_mean_fisher_z])
...
display([rho_reho_ied_lt_cat_pval_fisher_z rho_reho_ied_ds_cat_pval_fisher_z, ...
    ...
    rho_falff_ied_lt_cat_pval_fisher_z, rho_falff_ied_ds_cat_pval_fisher_z])
...
falff_ied_lt_xlabel_str = 'fALFF--IED\(_{\mathrm{long} \: \mathrm{term}}\)';
falff_ied_ds_xlabel_str = 'fALFF--IED\(_{\mathrm{during} \: \mathrm{scan}}\)';
...
if rho_falff_ied_lt_cat_pval_fisher_z <= thresh
    falff_ied_lt_xlabel_str = 'fALFF--IED\(_{\mathrm{long} \: \mathrm{term}}\)*';
end
if rho_falff_ied_ds_cat_pval_fisher_z <= thresh
    falff_ied_ds_xlabel_str = 'fALFF--IED\(_{\mathrm{during} \: \mathrm{scan}}\)*';
end

% format output xlabel str based on input arrays of p-values of mean fisher
% z scores
if ~isempty(rho_reho_ied_ds_cat_pval_fisher_z) && ...
        ~isempty(rho_alff_ied_ds_cat_pval_fisher_z) && ...
        ~isempty(rho_falff_ied_ds_cat_pval_fisher_z)
xlabel_str_output = {reho_ied_lt_xlabel_str, reho_ied_ds_xlabel_str, ...
    alff_ied_lt_xlabel_str, alff_ied_ds_xlabel_str, ...
    falff_ied_lt_xlabel_str, falff_ied_ds_xlabel_str};
else
xlabel_str_output = {reho_ied_lt_xlabel_str, alff_ied_lt_xlabel_str, ...
    falff_ied_lt_xlabel_str};
end
