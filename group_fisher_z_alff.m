...
% spearman's corr. coeff. btw. mean normalized alff and number of spikes,
...
op_filename = 'fisher_z_alff_all_subs.mat';
...
fname_alff_struct = [directname, filesep, 'matrices' filesep 'alff_spikes_fixed_winsize'];   % direct. of alff struct is located
filename_alff_struct = 'alff_spikes_fixed_windsize.mat';   % filename of alff struct. file
...
[~, alff_struct_path, ~] = get_path(fname_alff_struct, filename_alff_struct);   % alff
...
if isempty(alff_struct_path) || isempty(spikes_stats_struct_path)
    return
end
...
fds_alff = fieldnames(st_alff.terms);   % alff struct.
fds_spikes_stats = fieldnames(st_spikes_stats.terms);   % spikes stats struct.

% get non-empty fieldnames for alff and spikes_stats struct.
fd_ind_alff = [];   % initialize array for storing indices of non-empty fields
for sess_ind = 1:length(fds_alff)   % for each field
    if ~isempty(st_alff.terms.(fds_alff{sess_ind}))   % check if field is empty
        fd_ind_alff = [fd_ind_alff sess_ind];   % if not, append index to array
    end
end
...
fdnames_alff = fds_alff(fd_ind_alff);   % alff
fdnames_spikes_stats = fds_alff(fd_ind_spikes_stats);   % spikes stats

% variable interested: 
% spearman's rho and p-value btw. mean ALFF and number of spikes (A),
...
rho_pval_comb_all = st_alff.terms.(fdnames_alff{1}).rho_pval_comb_all;   % spearman rho and p-value
