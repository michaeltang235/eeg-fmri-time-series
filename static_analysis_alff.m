...
for run_ind = 1:numel(swra_file_path)   % for every session
    ...
    %---------------------------------------------------------------------------
    % Part (IX): get names and event types of prominent channels, defined 
    ...
    % %---------------------------------------------------------------------------
    % Part (X): get names of less-prominent channels relative to each
    ...
    % END Part (X): get names of less-prominent channels relative to each
    % type of prominent channels, and names of other channels
    % %---------------------------------------------------------------------------
    
    % Part (XI): assign ied rate (per minute) measured over entire scan 
    ...
    for row = 1:size(prom_ch, 1)   % for each row in prom_ch
        ...
        for i = 1:size(ied_during_scan, 1)   % for every row in ied_during_scan array
            ...
            if ismember(ev_type, ied_during_scan{i, 1})
                ...
                ch_name_matched_ied_alff(row_est, 1:2) = prom_ch(row, 1:2);   % event type and prom. channel name
                ch_name_matched_ied_alff{row_est, 3} = ied_during_scan{i, end};   % ied rate during scan
    ...
    % matching ied type reported in ied_during scan array
    row_est = size(ch_name_matched_ied_alff, 1);   % get num. of rows est. 
    ...
    for row = 1:size(less_prom_ch, 1)   % for each less-prom. ch.
        ...
        for i = 1:size(ied_during_scan, 1)   % for every row in ied_during_scan
            ...
            if ismember(ev_type, ied_during_scan{i, 1})   
                ...
                % and ied rate (per min.) measured over entire
                % scan to curr. channel
                ch_name_matched_ied_alff(row_num, 1:2) = less_prom_ch(row, :);   % event type and less-prom. ch. name
                ch_name_matched_ied_alff{row_num, 3}  = ied_during_scan{i, end};   % ied rate during scan
                row_num = row_num + 1;
            end
        end
    end
    ...
    for row = 1:size(other_ch, 1)   % for every chanenl in other_ch
        ch_name_matched_ied_alff{row_num, 1} = [];   % assign empty event type to col. 1
        ch_name_matched_ied_alff{row_num, 2} = other_ch{row};   % assign channel name
        ch_name_matched_ied_alff{row_num, 3} = 0;   % assign zero ied rate during scan to curr. ch.
    ...
    for row = 1:size(ch_name_matched_ied_alff, 1)   % for every row in ch_name_matched_ied_reho
        ...
        for i = 1:size(sig_box_all, 1)   % for every row in sig_box_all
            if isequal(ch_name_cur, sig_box_all{i, 2})   % find row with matching channel name
                ...
                ch_name_matched_ied_alff{row, ncol+1} = sig_box_all{i, end};
            end
        end
    end
    ...
    % END Part (XI): assign ied rate (per minute) measured over entire scan 
    % and reho to each channel that has ied rate registered during scan
    %---------------------------------------------------------------------------
    % Part (XII): get spearman's corr. coeff. btw. (i) ALFF and (ii) IED
    ...
    % END Part (XII): get spearman's corr. coeff. btw. (i) ALFF and (ii) IED
    % rate measured during long-term monitoring, and btw. (i) ALFF and
    % (iii) IED rate measured during scan (entire)
    %---------------------------------------------------------------------------
    ...
    % ied rate during scan and alff of every channel
