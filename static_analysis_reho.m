...
for run_ind = 1:numel(swra_file_path)   % for every session
    ...
    %---------------------------------------------------------------------------
    % Part (VIII): get names and coordinates of channels with ied rates
    ...
    % assign ied rate measured over entire scan to each channel with
    % matching ied type as reported in ied_during scan array
    ...
    for row = 1:size(ch_name_matched, 1)   % for each row in ch_name_matched
        ...
        for i = 1:size(ied_during_scan, 1)   % for every row in ied_during_scan array
            ...
            if ismember(ev_type, ied_during_scan{i, 1})
                ...
                ch_name_matched_ied_reho(row_est, 1:ncol) = ch_name_matched(row, :);
                ch_name_matched_ied_reho{row_est, ncol+1} = ied_during_scan{i, end};
    ...
    for row = 1:size(ch_name_matched_ied_reho, 1)   % for every row in ch_name_matched_ied_reho
        ...
        for i = 1:size(sig_box_all, 1)   % for every row in sig_box_all
            if isequal(ch_name_cur, sig_box_all{i, 2})   % find row with matching channel name
                ...
                ch_name_matched_ied_reho{row, ncol+2} = sig_box_all{i, end};
            end
        end
    end
...
%         [ch_name_matched_ied_reho{:, end}]', 'Type', 'Spearman');
    ...
    % only portions of channels have ied rates registered during scan, ch_name_matched_ied_reho
    % holds info about ied rate measured during scan and reho of those
    % channels
