...
% Updates:
% June 13, 2021
% filter motion artifacts from time series before computing avg. signals 
% and corr. coeff.
...
for i = 1:numel(midpt)   % for each electrode type
    for j = 1:size(midpt{i}, 1)   % for each electrode pair under current type
        ...
        signal_mo_filtered = filter_motion(motion_file_path, signal);
        sig_box{i}{j, 2} = signal_mo_filtered;
...
% get max. spearman's rho btw. corr. coeff. and spike rates
max_rho_cs_sp = max([rho_cs_sp{:}]);

% set thresh. for filtering entries  as max. (round to nearest 1 decimal
% place) obtained above
thresh_rho_cs_sp = floor(max_rho_cs_sp*10)/10;
