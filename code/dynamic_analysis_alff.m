...
filename_op = 'dynamic_analysis_alff.mat';   % filename of output file
...
for run_ind = 1:numel(swra_file_path)   % for every session
    ...
    % alff in every time segment to corresponding channel (ied onset, ied propagation, and others)

    % use match_ds_ied_alff, defined at the end of script, to assign during 
    % scan ied rate (per minute) and alff measured over entire scan to every 
    ...
    ch_onset_ds_ied_alff = match_ds_ied_alff(ch_list_onset, inst_ied, sig_box_all);
    ch_propa_ds_ied_alff = match_ds_ied_alff(ch_list_propa, inst_ied, sig_box_all);
    ch_ext_ds_ied_alff = match_ds_ied_alff(ch_list_ext, inst_ied, sig_box_all);

    % END Part (VII): assign instantaneous ied rate (per minute) and alff
    ...
    % ied rate and mean normalized alff of every time segment of every channel
    ...
    opstruct.ch_ext_ds_ied_alff = ch_ext_ds_ied_alff;   % ext = onset + propa
