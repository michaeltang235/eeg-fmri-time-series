...
filename_op = 'static_analysis_alff_1.mat';   % filename of output file
...
for run_ind = 1:numel(swra_file_path)   % for every session
    ...
    % END Part (II): get amplitude of low-frequency fluctuation (ALFF) 
    %---------------------------------------------------------------------------
    ...
    % use match_lt_ied_alff, defined at the end of script, to assigan long term
    % monitoring ied rate (per minute) and alff measured over entire scan to 
    % every channel listed in each of the arrays obtained above (ied onset, 
    % ied propagation, no ied, all = onset + propa + no ied)
    ch_onset_lt_ied_alff = match_lt_ied_alff(ch_list_onset, ied_long_term, sig_box_all);
    ch_propa_lt_ied_alff = match_lt_ied_alff(ch_list_propa, ied_long_term, sig_box_all);
    ch_noied_lt_ied_alff = match_lt_ied_alff(ch_list_noied, ied_long_term, sig_box_all);
    ch_all_lt_ied_alff = match_lt_ied_alff(ch_list_all, ied_long_term, sig_box_all);

    % use match_ds_ied_alff, defined at the end of script, to assign during 
    % scan ied rate (per minute) and alfff measured over entire scan to every 
    ...
    ch_onset_ds_ied_alff = match_ds_ied_alff(ch_list_onset, ied_during_scan, sig_box_all);
    ch_propa_ds_ied_alff = match_ds_ied_alff(ch_list_propa, ied_during_scan, sig_box_all);
    ch_ext_ds_ied_alff = match_ds_ied_alff(ch_list_ext, ied_during_scan, sig_box_all);
    ...
    % corr. btw. ALFF and ied measured during long-term monitoring and its 
    % p-value are classified into the following categories based on channel
    % type, onset, propa, no ied, and all,
    [rho_alff_ied_lt_onset, pval_alff_ied_lt_onset] = compute_corr(ch_onset_lt_ied_alff);
    [rho_alff_ied_lt_propa, pval_alff_ied_lt_propa] = compute_corr(ch_propa_lt_ied_alff);
    [rho_alff_ied_lt_noied, pval_alff_ied_lt_noied] = compute_corr(ch_noied_lt_ied_alff);
    [rho_alff_ied_lt_all, pval_alff_ied_lt_all] = compute_corr(ch_all_lt_ied_alff);

    % corr. btw. ALFF and ied measured during scan and its 
    % p-value are classified into the following categories based on channel
    % type, onset, propa, ext,
    % initialize arrays for each category
    [rho_alff_ied_ds_onset, pval_alff_ied_ds_onset] = compute_corr(ch_onset_ds_ied_alff);
    [rho_alff_ied_ds_propa, pval_alff_ied_ds_propa] = compute_corr(ch_propa_ds_ied_alff);
    [rho_alff_ied_ds_ext, pval_alff_ied_ds_ext] = compute_corr(ch_ext_ds_ied_alff);
    ...
    % arrays listing channels with mean normalized alff and long term 
    % monitoring ied rate per min assigned, divided into categories 
    opstruct.ch_onset_lt_ied_alff = ch_onset_lt_ied_alff;   % onset 
    opstruct.ch_propa_lt_ied_alff = ch_propa_lt_ied_alff;   % propa 
    opstruct.ch_noied_lt_ied_alff = ch_noied_lt_ied_alff;   % no ied 
    opstruct.ch_all_lt_ied_alff = ch_all_lt_ied_alff;   % all = onset+propa+no ied

    % arrays listing channels with mean normalized alff and during scan 
    % ied rate per min assigned, divided into categories 
    opstruct.ch_onset_ds_ied_alff = ch_onset_ds_ied_alff;   % onset 
    opstruct.ch_propa_ds_ied_alff = ch_propa_ds_ied_alff;   % propa 
    opstruct.ch_ext_ds_ied_alff = ch_ext_ds_ied_alff;   % no ied 
    
    % so, we have for every channel, (i) ALFF, (ii) IED rate measured
    ...
    opstruct.rho_alff_ied_lt_onset = rho_alff_ied_lt_onset;   % onset
    opstruct.pval_alff_ied_lt_onset = pval_alff_ied_lt_onset;

    opstruct.rho_alff_ied_lt_propa = rho_alff_ied_lt_propa;   % propa
    opstruct.pval_alff_ied_lt_propa = pval_alff_ied_lt_propa;

    opstruct.rho_alff_ied_lt_noied = rho_alff_ied_lt_noied;   % no ied
    opstruct.pval_alff_ied_lt_noied = pval_alff_ied_lt_noied;

    opstruct.rho_alff_ied_lt_all = rho_alff_ied_lt_all;   % all = onset+propa+no ied
    opstruct.pval_alff_ied_lt_all = pval_alff_ied_lt_all;
    
    % for spearman's rho btw. (i) and (iii) and its p-value, we have 
    opstruct.rho_alff_ied_ds_onset = rho_alff_ied_ds_onset;   % onset
    opstruct.pval_alff_ied_ds_onset = pval_alff_ied_ds_onset;

    opstruct.rho_alff_ied_ds_propa = rho_alff_ied_ds_propa;   % propa
    opstruct.pval_alff_ied_ds_propa = pval_alff_ied_ds_propa;

    opstruct.rho_alff_ied_ds_ext = rho_alff_ied_ds_ext;   % ext = onset+propa
    opstruct.pval_alff_ied_ds_ext = pval_alff_ied_ds_ext;
...
% FUNCTION OUTPUT_AR = MATCH_LT_IED_ALFF(INPUT_AR, IED_LT, SIG_BOX_ALL)
...
function output_ar = match_lt_ied_alff(input_ar, ied_lt, sig_box_all_ar)
...
for i = 1:size(ch_lt_ied, 1)   % for every row in ch_lt_ied
    ...
    for j = 1:size(sig_box_all_ar, 1)   % for every row in sig_box_all_ar
        if isequal(ch_name_cur, sig_box_all_ar{j, 2})   % find row with matching channel name
            ...
            ch_lt_ied_alff(row_num, 1:ncol) = ch_lt_ied(i, :);
            ch_lt_ied_alff{row_num, ncol+1} = sig_box_all_ar{j, end};
            row_num = row_num + 1;   % increment row number by 1 for next matched channel
        end
    end
end
...
output_ar = ch_lt_ied_alff;

end   % end function output_ar = match_lt_ied_alff(input_ar, ied_lt, sig_box_all_ar)

%----------------------------------------------------------------------------
%----------------------------------------------------------------------------

% FUNCTION OUTPUT_AR = MATCH_DS_IED_ALFF(INPUT_AR, IED_DS, SIG_BOX_ALL)
% this function requires the following inputs,
% INPUT_AR = array containing channels to which during scan ied and reho
% are assigned, with format given below
% 1st col. = subject number
...
function output_ar = match_ds_ied_alff(input_ar, ied_ds, sig_box_all_ar)
...
% initialize ch_ds_ied_alff, with the same format as
...
for i = 1:size(ch_ds_ied, 1)   % for every row in ch_ds_ied
    ...
    for j = 1:size(sig_box_all_ar, 1)   % for every row in sig_box_all_ar
        if isequal(ch_name_cur, sig_box_all_ar{j, 2})   % find row with matching channel name
            % assign alff of matched channel to rightmost col. of current row
            ch_ds_ied_alff(row_num, 1:ncol) = ch_ds_ied(i, :);
            ch_ds_ied_alff{row_num, ncol+1} = sig_box_all_ar{j, end};
            row_num = row_num + 1;   % increment row number by 1 for next matched channel
        end
    end
end
...
% assign variable computed to output
output_ar = ch_ds_ied_alff;

end   % end function output_ar = match_ds_ied_alff(input_ar, ied_ds, sig_box_all_ar)
