...
% (i) combine instantaneous IED rate (A) and ReHo (B) computed in every time 
...
fname_input = [subnum_dir, filesep, 'matrices' filesep 'dynamic_analysis_reho'];   % direct. of output matrix
filename_input = 'dynamic_analysis_reho.mat';   % filename of input file

% enter output filename
fname_output = fname_input;
filename_output = 'dynamic_analysis_reho_fisher_z.mat';
...
for sub_ind = 1:numel(fd_sub)
    ...
    % fmri scan)
    ch_onset_ds_ied_reho_comb = combine_ar(st_reho.grand.(fd_sub{sub_ind}), 'ch_onset_ds_ied_reho');
    ch_propa_ds_ied_reho_comb = combine_ar(st_reho.grand.(fd_sub{sub_ind}), 'ch_propa_ds_ied_reho');
    ch_ext_ds_ied_reho_comb = combine_ar(st_reho.grand.(fd_sub{sub_ind}), 'ch_ext_ds_ied_reho');

    % store arrays to output struct under curr. subject fieldname
    main.(fd_sub{sub_ind}).ch_onset_ds_ied_reho_comb = ch_onset_ds_ied_reho_comb;
    main.(fd_sub{sub_ind}).ch_propa_ds_ied_reho_comb = ch_propa_ds_ied_reho_comb;
    main.(fd_sub{sub_ind}).ch_ext_ds_ied_reho_comb = ch_ext_ds_ied_reho_comb;
