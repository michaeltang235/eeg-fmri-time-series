...
fname_op = [subnum_dir, filesep, 'matrices' filesep 'dynamic_analysis_fc'];   % direct. of output matrix
filename_op = 'dynamic_analysis_fc.mat';   % filename of output file
...
for run_ind = 1:numel(swra_file_path)   % for every session
    ...
    % estimate. If so, pass control to next iteration (next run)
    ...
    % obtain time axis, list of initial and final times in seconds (ti and tf)
    % of time series in current run
    taxis = inst_ied_st.taxis;
    ti_list = inst_ied_st.ti_list;
    tf_list = inst_ied_st.tf_list;

    % END Part (IV): obtain instantaneous ied rate [/min] of every event type
    % in current session
    %---------------------------------------------------------------------------

    % Part (V): remove noise and get avg. siganl of each segment of every channel
    ...
    % % 2nd column lists avg. signal in each segment in 3X3X3 box of every channel
    sig_box_all = {};
    ...
    for item = 1:size(midpt_all, 1)
        ...
        for seg_ind = 1:length(ti_list)   % for every segment
            ...
            % sig_box
            sig_box_all{item, 2}{seg_ind} = avg_signal(:, ti_ind:tf_ind);
    
        end   % end for seg_ind = 1:length(ti_list)   % for every segment
    
    end   % end for item = 1:size(midpt_all, 1)

    % END Part (VII): remove noise and get avg. siganl of every channel
    %---------------------------------------------------------------------------

    % Part (VIII): format pairs of IED onset, IED propagation, and
    % no IED channels
    ...
    % END Part (VIII): assign avg. signal to IED onset, IED propagation, and
    % no IED channels
    %---------------------------------------------------------------------------
    
    % Part (IX): compute functional connectivity btw. each segment of every channel pair
    ...
    % 8th col. = functional connectivity btw. every segment 
    % 9th col. = coef. of var. (std. dev./mean) of inst. ied rate [/min] of ref. channel
    % 10th col. = coef. of var. (std. dev./mean) of inst. ied rate [/min] of target channel
    ...
    for i = 1:size(ch_pair, 1)   % for each row in ch_pair array
        ...
        ref_ied_type = ch_pair{i, 2};   % ied type of ref. ch.
        targ_ied_type = ch_pair{i, 5};   % ied type of target ch.
        ...
        if ~isempty(ind_ref_name) && ~isempty(ind_targ_name)
            ...
            % obtain info from all col. est.
            fc_ar(row_num, 1:ncol) = ch_pair(i, :);

            % compute fc for current pair in each segment
            for seg_ind = 1:numel(ti_list)
            fc = corr(sig_box_all{ind_ref_name, end}{seg_ind}', ...
                sig_box_all{ind_targ_name, end}{seg_ind}', 'Type', 'Pearson');
            fc_ar{row_num, ncol+1}{seg_ind} = fc;
            end

            % obtain inst. ied rate [/min] from inst_ied array based on ref
            % and targ ied types by finding matching row in inst_ied array
            ref_inst_ied_rate = [];   % initialize array, remain empty if no ied is found
            targ_inst_ied_rate = [];
            for row = 1:size(inst_ied)
                if ismember(str2num(ref_ied_type), inst_ied{row, 1})
                    ref_inst_ied_rate = inst_ied{row, end};
                end
            end

            for row = 1:size(inst_ied)
                if ismember(str2num(targ_ied_type), inst_ied{row, 1})
                    targ_inst_ied_rate = inst_ied{row, end};
                end
            end

            % assign inst. ied rates [/min] of ref. and target channels to fc_ar
            fc_ar{row_num, ncol+2} = ref_inst_ied_rate;
            fc_ar{row_num, ncol+3} = targ_inst_ied_rate;

            row_num = row_num + 1;   % increment row number by 1 for next ch. pair
        end
    end

    % END Part (IX): compute functional connectivity btw. each segment of every channel pair
    %---------------------------------------------------------------------------

    % Part (X): store quantities calculated in current run to struct
    ...
    % END Part (X): store quantities calculated in current run to struct
    %---------------------------------------------------------------------------
