...
% (i) combine instantaneous IED rate (A) and ffalff (B) computed in every time 
% segment of each channel across sessions ....
% (ii) compute Spearman's corr. rehocoeff. btw. the combined (A) and (B)
...
fname_input = [subnum_dir, filesep, 'matrices' filesep 'dynamic_analysis_falff'];   % direct. of output matrix
filename_input = 'dynamic_analysis_falff.mat';   % filename of input file

% enter output filename
fname_output = fname_input;
filename_output = 'dynamic_analysis_falff_fisher_z.mat';
...
[~, falff_struct_path, ~] = get_path(fname_input, filename_input);   % reho

% load sturct. from path to variable
st_falff = load(falff_struct_path{:});   % falff struct.

% obtain fieldnames of subjects
fd_sub = fieldnames(st_falff.grand);
...
for sub_ind = 1:numel(fd_sub)
    ...
    % fmri scan)
    ch_onset_ds_ied_falff_comb = combine_ar(st_falff.grand.(fd_sub{sub_ind}), 'ch_onset_ds_ied_falff');
    ch_propa_ds_ied_falff_comb = combine_ar(st_falff.grand.(fd_sub{sub_ind}), 'ch_propa_ds_ied_falff');
    ch_ext_ds_ied_falff_comb = combine_ar(st_falff.grand.(fd_sub{sub_ind}), 'ch_ext_ds_ied_falff');

    % store arrays to output struct under curr. subject fieldname
    main.(fd_sub{sub_ind}).ch_onset_ds_ied_falff_comb = ch_onset_ds_ied_falff_comb;
    main.(fd_sub{sub_ind}).ch_propa_ds_ied_falff_comb = ch_propa_ds_ied_falff_comb;
    main.(fd_sub{sub_ind}).ch_ext_ds_ied_falff_comb = ch_ext_ds_ied_falff_comb;
...
% output_ar = output array with ied rate and feature (reho/falff) in each
...
for i = 1:size(ar_int_all, 1)
    ...
    if isequal(ied_type_ch_found, 0)
        ...
        feat_comb = [];   % feature (reho/falff) in each segment across sessions
        ...
        % sessions (A)
        % 5th col. = feature (reho/falff) in every segment combined from all sessions (B)
