...
% (i) get fractional ALFF over entire scan for each channel in every session
...
% (B): compute (i) frac. ALFF and (ii) IED rate measured during long-term monitoring,
...
fname_op = [subnum_dir, filesep, 'matrices' filesep 'static_analysis_falff'];   % direct. of output matrix
filename_op = 'static_analysis_falff_1.mat';   % filename of output file
...
for run_ind = 1:numel(swra_file_path)   % for every session
    ...
    % use get_falff to get map of normalized fractional alff
    norm_falff = get_falff(motion_filtered_img, expmask, tr, []);

    % if empty normalized frac. alff map is obtained, time series of current 
    % session is not long enough for making psd estimates using pwelch method, 
    ...
    % END Part (II): get fractional amplitude of low-frequency fluctuation (fALFF) 
    %---------------------------------------------------------------------------
    ...
    for i = 1:numel(midpt)   % for each electrode type
        for j = 1:size(midpt{i}, 1)   % for each electrode pair under current type
            ...
            % get mean normalized fractional alff (fALFF) in current 3X3X3 box, 
            % assign result to 3rd column of current cell within sig_box
            if ~isempty(norm_falff)   % if norm_falff array is not empty
                
                % get number of non-zero entries in current regional mask
                num_nonzero = nnz(reg_mask);
            
                % get mean normalized frac. alff in current 3X3X3 box
                sig_box{i}{j, 2} = sum(sum(sum(norm_falff(mxyz(1)-1:mxyz(1)+1, ...
                    mxyz(2)-1:mxyz(2)+1, mxyz(3)-1:mxyz(3)+1))))/num_nonzero;
            else
                sig_box{i}{j, 2} = [];   % assign empty value if norm_falff is empty
            end
    ...
    % col. 3 = mean normalized fractional alff of every channel
    ...
    %---------------------------------------------------------------------------
    % Part (IX): get names and event types of ied onset and ied
    ...
    % and mean normalized fractional ALFF to every channel in each array 
    % (ied onset, propagation, no ied, all)

    % use match_lt_ied_falff, defined at the end of script, to assigan long term
    % monitoring ied rate (per minute) and frac. alff measured over entire scan to 
    % every channel listed in each of the arrays obtained above (ied onset, 
    % ied propagation, no ied, all = onset + propa + no ied)
    ch_onset_lt_ied_falff = match_lt_ied_falff(ch_list_onset, ied_long_term, sig_box_all);
    ch_propa_lt_ied_falff = match_lt_ied_falff(ch_list_propa, ied_long_term, sig_box_all);
    ch_noied_lt_ied_falff = match_lt_ied_falff(ch_list_noied, ied_long_term, sig_box_all);
    ch_all_lt_ied_falff = match_lt_ied_falff(ch_list_all, ied_long_term, sig_box_all);
    ...
    ch_onset_ds_ied_falff = match_ds_ied_falff(ch_list_onset, ied_during_scan, sig_box_all);
    ch_propa_ds_ied_falff = match_ds_ied_falff(ch_list_propa, ied_during_scan, sig_box_all);
    ch_ext_ds_ied_falff = match_ds_ied_falff(ch_list_ext, ied_during_scan, sig_box_all);

    % END Part (X): assign ied rate (per minute) measured over entire scan 
    % and mean normalized ALFF to every channel in each array 
    % (ied onset, propagation, no ied, all)
    %---------------------------------------------------------------------------

    % Part (XI): get spearman's corr. coeff. btw. (i) ALFF and (ii) IED
    % rate measured during long-term monitoring, and btw. (i) ALFF and
    % (iii) IED rate measured during scan (entire)
    
    % corr. btw. frac. ALFF and ied measured during long-term monitoring and its 
    % p-value are classified into the following categories based on channel
    % type, onset, propa, no ied, and all,
    [rho_falff_ied_lt_onset, pval_falff_ied_lt_onset] = compute_corr(ch_onset_lt_ied_falff);
    [rho_falff_ied_lt_propa, pval_falff_ied_lt_propa] = compute_corr(ch_propa_lt_ied_falff);
    [rho_falff_ied_lt_noied, pval_falff_ied_lt_noied] = compute_corr(ch_noied_lt_ied_falff);
    [rho_falff_ied_lt_all, pval_falff_ied_lt_all] = compute_corr(ch_all_lt_ied_falff);

    % corr. btw. frac. ALFF and ied measured during scan and its 
    % p-value are classified into the following categories based on channel
    % type, onset, propa, ext,
    % initialize arrays for each category
    [rho_falff_ied_ds_onset, pval_falff_ied_ds_onset] = compute_corr(ch_onset_ds_ied_falff);
    [rho_falff_ied_ds_propa, pval_falff_ied_ds_propa] = compute_corr(ch_propa_ds_ied_falff);
    [rho_falff_ied_ds_ext, pval_falff_ied_ds_ext] = compute_corr(ch_ext_ds_ied_falff);

    % END Part (XI): get spearman's corr. coeff. btw. (i) ALFF and (ii) IED
    % rate measured during long-term monitoring, and btw. (i) ALFF and
    % (iii) IED rate measured during scan (entire)
    %---------------------------------------------------------------------------
    ...
    % arrays listing channels with mean normalized frac. alff and long term 
    % monitoring ied rate per min assigned, divided into categories 
    opstruct.ch_onset_lt_ied_falff = ch_onset_lt_ied_falff;   % onset 
    opstruct.ch_propa_lt_ied_falff = ch_propa_lt_ied_falff;   % propa 
    opstruct.ch_noied_lt_ied_falff = ch_noied_lt_ied_falff;   % no ied 
    opstruct.ch_all_lt_ied_falff = ch_all_lt_ied_falff;   % all = onset+propa+no ied

    % arrays listing channels with mean normalized frac. alff and during scan 
    % ied rate per min assigned, divided into categories 
    opstruct.ch_onset_ds_ied_falff = ch_onset_ds_ied_falff;   % onset 
    opstruct.ch_propa_ds_ied_falff = ch_propa_ds_ied_falff;   % propa 
    opstruct.ch_ext_ds_ied_falff = ch_ext_ds_ied_falff;   % no ied 
    
    % so, we have for every channel, (i) frac. ALFF, (ii) IED rate measured
    ...
    opstruct.rho_falff_ied_lt_onset = rho_falff_ied_lt_onset;   % onset
    opstruct.pval_falff_ied_lt_onset = pval_falff_ied_lt_onset;

    opstruct.rho_falff_ied_lt_propa = rho_falff_ied_lt_propa;   % propa
    opstruct.pval_falff_ied_lt_propa = pval_falff_ied_lt_propa;

    opstruct.rho_falff_ied_lt_noied = rho_falff_ied_lt_noied;   % no ied
    opstruct.pval_falff_ied_lt_noied = pval_falff_ied_lt_noied;

    opstruct.rho_falff_ied_lt_all = rho_falff_ied_lt_all;   % all = onset+propa+no ied
    opstruct.pval_falff_ied_lt_all = pval_falff_ied_lt_all;
    
    % for spearman's rho btw. (i) and (iii) and its p-value, we have 
    opstruct.rho_falff_ied_ds_onset = rho_falff_ied_ds_onset;   % onset
    opstruct.pval_falff_ied_ds_onset = pval_falff_ied_ds_onset;

    opstruct.rho_falff_ied_ds_propa = rho_falff_ied_ds_propa;   % propa
    opstruct.pval_falff_ied_ds_propa = pval_falff_ied_ds_propa;

    opstruct.rho_falff_ied_ds_ext = rho_falff_ied_ds_ext;   % ext = onset+propa
    opstruct.pval_falff_ied_ds_ext = pval_falff_ied_ds_ext;
...
% FUNCTION OUTPUT_AR = MATCH_LT_IED_FALFF(INPUT_AR, IED_LT, SIG_BOX_ALL)
% this function requires the following inputs,
% INPUT_AR = array containing channels to which during scan ied and
% reho/alfff are assigned, with format given below
...
% OUTPUT_AR = output array with during scan ied rate and frac. ALFF assigned 
% to each channel listed in INPUT_AR, with format given below
% 1st col. = subject number
% 2nd col. = ied type 
% 3rd col. = channel name
...
function output_ar = match_lt_ied_falff(input_ar, ied_lt, sig_box_all_ar)
...
% initialize ch_lt_ied_alff, with the same format as
% ch_lt_ied, but with an additional column storing frac. alff
...
for i = 1:size(ch_lt_ied, 1)   % for every row in ch_lt_ied
    ...
    for j = 1:size(sig_box_all_ar, 1)   % for every row in sig_box_all_ar
        if isequal(ch_name_cur, sig_box_all_ar{j, 2})   % find row with matching channel name
            % assign frac. alff of matched channel to rightmost col. of current row
            ch_lt_ied_falff(row_num, 1:ncol) = ch_lt_ied(i, :);
            ch_lt_ied_falff{row_num, ncol+1} = sig_box_all_ar{j, end};
            row_num = row_num + 1;   % increment row number by 1 for next matched channel
        end
    end
end
...
output_ar = ch_lt_ied_falff;

end   % end function output_ar = match_lt_ied_falff(input_ar, ied_lt, sig_box_all_ar)
...
% assign alff measured over entire scan to each channel that has
% matched ied type found, and matched channel name in sig_box_all_ar
